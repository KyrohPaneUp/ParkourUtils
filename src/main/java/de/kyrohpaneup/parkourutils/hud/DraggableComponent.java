...
public class DraggableComponent {
    ...
    public void draw(int mouseX, int mouseY){
        ...
        if (mouseOverX && mouseOverY && Mouse.isButtonDown(0)) {
            if (HUDConfigScreen.currentlyDragging == null) {
                this.lastX = x - mouseX;
                this.lastY = y - mouseY;
                this.dragging = true;
                HUDConfigScreen.currentlyDragging = this;
            }
        }
    }

    private void draggingFix(int mouseX, int mouseY) {
        if (HUDConfigScreen.currentlyDragging == this && this.dragging) {
            this.x = mouseX + this.lastX;
            this.y = mouseY + this.lastY;

            if (!Mouse.isButtonDown(0)) {
                this.dragging = false;
                HUDConfigScreen.currentlyDragging = null;
            }
        }
    }
}
